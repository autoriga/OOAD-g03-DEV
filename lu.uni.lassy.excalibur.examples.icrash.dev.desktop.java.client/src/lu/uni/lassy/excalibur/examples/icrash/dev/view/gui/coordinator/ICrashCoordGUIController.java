...
public class ICrashCoordGUIController extends AbstractAuthGUIController {
	...
	*/
	...
	@FXML
	void bttnKnowRate_OnClick(ActionEvent event) {
	    	
    	rw = new RateWindow(userController.getRate());
    	rw.setVisible(true);
    }
	...
	private void closeCrisis(){
		...
		if (crisis != null)
		{
			try {
				...
				userController.incAll();
				if (userController.ifAddRate()) {
					
					userController.incRate();
					userController.zeroAll();
				}
			} catch (ServerOfflineException | ServerNotBoundException e) {
				showServerOffLineMessage(e);
			} catch (IncorrectFormatException e) {
	...
	protected void logonShowPanes(boolean loggedOn){
		...
		bttnKnowRate.setDisable(!loggedOn);
